...
def create_service_order_internal(db: Session, order: service_schema.ServiceOrderCreate) -> int:
    ...
    try:
        ...
        # Use simple format strings instead of named params if using raw execution without mapping
        # Since we don't have SQLAlchemy models mapped yet, we use raw execute
        result = db.execute(
            insert_order_query,
            order_params
        )
...
def get_service_order(order_id: int, db: Session = Depends(get_db)):
    ...
    """
    order_query = "SELECT * FROM Service_Order WHERE ServiceOrderId = :order_id AND IsDeleted = 0"
    order_row = db.execute(order_query, {"order_id": order_id}).fetchone()
    
    if not order_row:
        raise HTTPException(status_code=404, detail="Service Order not found")
        
    items_query = "SELECT * FROM Service_OrderItem WHERE ServiceOrderId = :order_id AND IsDeleted = 0"
...
def list_service_orders(db: Session = Depends(get_db)):
    ...
    """
    orders_query = "SELECT * FROM Service_Order WHERE IsDeleted = 0 ORDER BY CreatedAt DESC LIMIT 50"
