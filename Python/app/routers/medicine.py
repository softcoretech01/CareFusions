...
def _call_sp(db: Session, opt: str, medicine_id: int = 0, **kwargs):
    params = {
        ...
        "p_ManufacturerId": kwargs.get("manufacturer_id") if kwargs.get("manufacturer_id") is not None else 0,
        "p_CategoryId":     kwargs.get("category_id") if kwargs.get("category_id") is not None else 0,
        "p_SubCategoryId":  kwargs.get("sub_category_id") if kwargs.get("sub_category_id") is not None else 0,
    ...
    sql = text(f"""
        CALL {SP_NAME}(
            :p_Opt, :p_MedicineId,
            :p_MedicineCode, :p_GenericName, :p_CategoryId, :p_SubCategoryId, :p_ManufacturerId,
...
def create(medicine: MedicineCreate, db: Session = Depends(get_db)):
    try:
        ...
        result = _call_sp(db, "INSERT",
            ...
            sub_category_id=d.get("subCategoryId"),
