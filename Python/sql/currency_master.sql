...
CREATE TABLE IF NOT EXISTS Master_Currency (
    ...
    -- Units of this currency per one unit of the base currency. The base
    -- currency itself is always 1 and the SP enforces that.
    ...
    KEY IDX_Currency_Base     (IsBaseCurrency),
...
CREATE PROCEDURE SpMasterCurrency(
    ...
    IN  p_ExchangeRate DECIMAL(14,6),
    IN  p_IsBase       TINYINT,
...
BEGIN
    ...
    IF p_Opt = 'GET' THEN
        SELECT
            CurrencyId, CurrencyCode, CurrencyName, Symbol,
            ExchangeRate, IsBaseCurrency, Status,
    ...
    ELSEIF p_Opt = 'INSERT' THEN
        ...
        -- A conversion rate of zero or less would silently zero out every
        -- amount converted through it.
        IF p_IsBase = 0 AND (p_ExchangeRate IS NULL OR p_ExchangeRate <= 0) THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'INVALID_EXCHANGE_RATE';
        END IF;

        -- Exactly one base currency: the incoming one demotes the old one, and
        -- the base is 1 by definition.
        IF p_IsBase = 1 THEN
            UPDATE Master_Currency SET IsBaseCurrency = 0 WHERE IsBaseCurrency = 1;
        END IF;

        INSERT INTO Master_Currency (
            CurrencyCode, CurrencyName, Symbol, ExchangeRate, IsBaseCurrency, Status, CreatedBy
        ) VALUES (
            p_CurrencyCode, p_CurrencyName, p_Symbol,
            CASE WHEN p_IsBase = 1 THEN 1.000000 ELSE p_ExchangeRate END,
            p_IsBase, p_Status, p_CreatedBy
        );
    ...
    ELSEIF p_Opt = 'UPDATE' THEN
        ...
        -- Clearing the flag on the only base currency would leave every rate
        -- without a reference point.
        IF p_IsBase = 0
           AND EXISTS (SELECT 1 FROM Master_Currency
                        WHERE CurrencyId = p_CurrencyId AND IsBaseCurrency = 1)
           AND NOT EXISTS (SELECT 1 FROM Master_Currency
                            WHERE IsBaseCurrency = 1 AND IsDeleted = 0
                              AND CurrencyId <> p_CurrencyId) THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BASE_CURRENCY_REQUIRED';
        END IF;

        IF p_IsBase = 1 THEN
            UPDATE Master_Currency SET IsBaseCurrency = 0
             WHERE IsBaseCurrency = 1 AND CurrencyId <> p_CurrencyId;
        END IF;
        ...
        SET
            ...
            Symbol         = p_Symbol,
            ExchangeRate   = CASE WHEN p_IsBase = 1 THEN 1.000000 ELSE p_ExchangeRate END,
            IsBaseCurrency = p_IsBase,
            Status         = p_Status,
    ...
    ELSEIF p_Opt = 'TOGGLESTATUS' THEN
        IF EXISTS (SELECT 1 FROM Master_Currency
                    WHERE CurrencyId = p_CurrencyId AND IsBaseCurrency = 1 AND Status = 'Active') THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BASE_CURRENCY_LOCKED';
        END IF;
    ...
    ELSEIF p_Opt = 'DELETE' THEN
        -- Deleting the base currency would orphan every other rate.
        IF EXISTS (SELECT 1 FROM Master_Currency
                    WHERE CurrencyId = p_CurrencyId AND IsBaseCurrency = 1 AND IsDeleted = 0) THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BASE_CURRENCY_LOCKED';
        END IF;
