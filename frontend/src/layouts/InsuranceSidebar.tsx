...
import { LayoutDashboard, ShieldCheck, ClipboardCheck, FileText, Gavel, Banknote } from 'lucide-react';
import { motion } from 'framer-motion';
import { useAppDispatch } from '../hooks/redux';
import { logout } from '../redux/slices/authSlice';

const navigation = [
  { name: 'Dashboard', to: '/insurance', icon: LayoutDashboard },
...
export const InsuranceSidebar = () => {
  ...
  // Clear the auth session, not just redirect — a bare navigate left the user
  // logged in, so the back button walked straight back into the app.
  const handleLogout = () => {
    dispatch(logout());
    navigate('/login');
  };
