...
import {
  LayoutDashboard, BedDouble, UserPlus, Users,
...
export const IPDSidebar = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const dispatch = useAppDispatch();
  const { wards } = useIPD();
  
  const [openSubmenu, setOpenSubmenu] = useState<string | null>('Active Admissions');

  const navigation = [
    { name: 'Dashboard', to: '/ipd', icon: LayoutDashboard },
    { name: 'Bed Management', to: '/ipd/bed-management', icon: BedDouble },
    { name: 'Admission Desk', to: '/ipd/admission-desk', icon: UserPlus },
    { 
      name: 'Active Admissions', 
      icon: Users,
      to: '/ipd/inpatients',
      submenus: [
        ...wards.map(w => ({ name: w.name, to: `/ipd/inpatients?ward=${w.id}` }))
      ]
    },
    { name: 'Ward Transfers', to: '/ipd/transfers', icon: ArrowRightLeft },
    { name: 'Discharges', to: '/ipd/discharges', icon: FileText },
  ];
        ...
        {navigation.map((item) => {
              ...
              <div key={item.name} className="space-y-1">
                <button
                  onClick={() => {
                    ...
                    if (!isOpen) navigate(item.to);
                  }}
                  className={`w-full flex items-center justify-between px-3 py-3 rounded-xl transition-all duration-200 group ${isOpen ? 'bg-white/10 text-white font-semibold' : 'hover:bg-white/5 hover:text-white text-white/80'
                    }`}
                >
                  <div className="flex items-center gap-3">
                    <Icon className={`w-5 h-5 shrink-0 transition-colors ${isOpen ? 'text-secondary' : 'text-white/60 group-hover:text-secondary'}`} />
                    <span className="text-sm">{item.name}</span>
                  </div>
                  <ChevronDown className={`w-4 h-4 transition-transform ${isOpen ? 'rotate-180 text-secondary' : 'text-white/40'}`} />
                </button>
          ...
          return (
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon className={`w-5 h-5 shrink-0 transition-colors ${isActive ? 'text-secondary' : 'text-white/60 group-hover:text-secondary'}`} />
                  <span className="text-sm">{item.name}</span>
                </>
              )}
            </NavLink>
