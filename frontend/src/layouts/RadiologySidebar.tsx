...
import {
  LayoutDashboard, ScanLine, AlertTriangle, FileText, Settings
} from 'lucide-react';
import { motion } from 'framer-motion';

const navigation = [
  { name: 'Dashboard', to: '/radiology/dashboard', icon: LayoutDashboard },
  { name: 'Scan Worklist', to: '/radiology/worklist', icon: ScanLine },
  { name: 'Critical Findings', to: '/radiology/alerts', icon: AlertTriangle },
  { name: 'Quality Control', to: '/radiology/qc', icon: Settings },
  { name: 'Reports', to: '/radiology/reports', icon: FileText },
];
...
export const RadiologySidebar = () => {
  return (
    <motion.aside
      initial={{ width: 80 }}
      animate={{ width: 300 }}
      className="h-screen bg-sidebar text-white/90 flex flex-col sticky top-0 overflow-hidden shadow-xl z-20"
    >
      {/* Logo Header */}
      ...
      <nav className="flex-1 overflow-y-auto custom-scrollbar px-3 py-6 space-y-1">
        {navigation.map((item) => (
          <NavLink
            key={item.name}
            to={item.to}
            end={item.to === '/radiology/dashboard'}
            className={({ isActive }) =>
          ...
          >
            {({ isActive }) => (
              <>
                <item.icon className={`w-5 h-5 transition-colors ${isActive ? 'text-secondary' : 'text-white/60 group-hover:text-secondary'}`} />
