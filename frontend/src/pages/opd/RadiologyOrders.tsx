          ...
          <table className="w-full">
            <thead className="bg-white sticky top-0 z-10 shadow-sm outline outline-1 outline-slate-100 text-xs font-semibold text-slate-500 uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4 text-left">Scans Ordered</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {visitsWithRadiology.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-slate-400">
                    No radiology orders found for the selected dates.
                  </td>
                </tr>
              ) : (
                visitsWithRadiology.map(visit => (
                  <tr key={visit.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-1">
                        {visit.radiologyOrders.map((order: any, idx: number) => {
                          const realStatus = getRadStatus(visit.uhid, order.serviceName, order.bodyPart, order.status, visit.date);
                          return (
                          <div key={idx} className="flex flex-col">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-bold text-slate-700">{order.bodyPart}</span>
                              <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded ${
                                realStatus === 'Ordered' ? 'bg-amber-100 text-amber-700' :
                                realStatus === 'Scheduled' ? 'bg-blue-100 text-blue-700' :
                                'bg-green-100 text-green-700'
                              }`}>
                                {realStatus}
                              </span>
                            </div>
                            <span className="text-xs text-slate-500">{order.modality}</span>
                          </div>
                          );
                        })}
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className={`flex items-center justify-center w-7 h-7 rounded-lg transition-colors ${
                            visit.radiologyOrders.some((o: any) => getRadStatus(visit.uhid, o.serviceName, o.bodyPart, o.status, visit.date) === 'Completed')
