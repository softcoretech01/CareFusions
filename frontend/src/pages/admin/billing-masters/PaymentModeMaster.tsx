...
const emptyData: Omit<PaymentModeRecord, 'id'> = {
  paymentCode: '',
  paymentMode: '',
  description: '',
...
const mockData: PaymentModeRecord[] = [
  { id: 1, paymentCode: 'PAY-001', paymentMode: 'Cash', description: 'Cash payments', transactionRequired: false, supportsRefund: true, isDefault: true, status: 'Active', remarks: '' },
  { id: 2, paymentCode: 'PAY-002', paymentMode: 'Credit Card', description: 'Credit Card payments', transactionRequired: true, supportsRefund: true, isDefault: false, status: 'Active', remarks: '' },
  { id: 3, paymentCode: 'PAY-003', paymentMode: 'Debit Card', description: 'Debit Card payments', transactionRequired: true, supportsRefund: true, isDefault: false, status: 'Active', remarks: '' },
  { id: 4, paymentCode: 'PAY-004', paymentMode: 'UPI', description: 'Unified Payments Interface', transactionRequired: true, supportsRefund: true, isDefault: false, status: 'Active', remarks: '' },
  { id: 5, paymentCode: 'PAY-005', paymentMode: 'Net Banking', description: 'Online banking transfer', transactionRequired: true, supportsRefund: true, isDefault: false, status: 'Active', remarks: '' },
  { id: 6, paymentCode: 'PAY-006', paymentMode: 'Insurance', description: 'Insurance coverage', transactionRequired: true, supportsRefund: false, isDefault: false, status: 'Active', remarks: 'Requires TPA/Insurance provider selection' },
  { id: 7, paymentCode: 'PAY-007', paymentMode: 'Corporate Credit', description: 'Corporate billing', transactionRequired: true, supportsRefund: false, isDefault: false, status: 'Active', remarks: 'Requires corporate mapping' }
];

export const PaymentModeMaster = () => {
  const [records, setRecords] = useState<PaymentModeRecord[]>(mockData);
  ...
  const [formData, setFormData] = useState<Omit<PaymentModeRecord, 'id'>>(emptyData);
  const [errors, setErrors] = useState<Record<string, string>>({});

  const validateForm = () => {
    const newErrors: Record<string, string> = {};
    if (!formData.paymentCode.trim()) newErrors.paymentCode = 'Payment Code is required';
    if (!formData.paymentMode.trim()) newErrors.paymentMode = 'Payment Mode is required';

    // Uniqueness checks
    if (records.some(r => r.paymentCode.toLowerCase() === formData.paymentCode.toLowerCase() && r.id !== selectedRecord?.id)) {
      newErrors.paymentCode = 'Payment Code must be unique';
    }
    if (records.some(r => r.paymentMode.toLowerCase() === formData.paymentMode.toLowerCase() && r.id !== selectedRecord?.id)) {
      newErrors.paymentMode = 'Payment Mode cannot be duplicated';
    }

    if (formData.isDefault) {
      if (records.some(r => r.isDefault && r.id !== selectedRecord?.id)) {
        newErrors.isDefault = 'Only one default payment mode is allowed. Uncheck the other first.';
      }
    }
  ...
  const handleCreateNew = () => {
    ...
    setFormData({
      ...emptyData,
      paymentCode: `PAY-${nextId.toString().padStart(3, '0')}`
    });
    setErrors({});
    setIsFormOpen(true);
  };
        ...
        <>
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-3">
              <Button variant="outline" icon={Download} onClick={() => exportToExcel(records, 'PaymentModeMaster')}>Export</Button>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Payment Code <span className="text-red-500">*</span></label>
                    <input type="text" value={formData.paymentCode} readOnly className="w-full px-4 py-2 bg-slate-100 border border-slate-200 rounded-xl text-sm text-slate-500 cursor-not-allowed focus:outline-none" />
                    {errors.paymentCode && <p className="text-red-500 text-xs mt-1">{errors.paymentCode}</p>}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Payment Mode <span className="text-red-500">*</span></label>
                    <input type="text" value={formData.paymentMode} onChange={e => setFormData({...formData, paymentMode: e.target.value})} className={`w-full px-4 py-2 bg-slate-50 border rounded-xl text-sm focus:outline-none focus:ring-2 transition-all ${errors.paymentMode ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-primary/20'}`} />
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="flex items-center gap-2">
                    ...
                    <label htmlFor="transactionRequired" className="text-sm font-medium text-slate-700">Transaction Required <span className="text-red-500">*</span></label>
                  </div>
                  <div className="flex items-center gap-2">
