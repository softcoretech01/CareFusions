...
interface MedicineCategoryRecord {
  id: number;
  categoryCode: string;
  categoryName: string;
  description: string;
...
const emptyData: Omit<MedicineCategoryRecord, 'id'> = {
  categoryCode: '',
  categoryName: '',
  description: '',
  status: 'Active',
...
export const MedicineCategoryMaster = () => {
  const [records, setRecords] = useState<MedicineCategoryRecord[]>([]);
  ...
  const validateForm = () => {
    ...
    // Uniqueness checks
    if (records.some(r => r.categoryCode.toLowerCase() === formData.categoryCode.toLowerCase() && r.id !== selectedRecord?.id)) {
      newErrors.categoryCode = 'Category Code must be unique';
    }
    if (records.some(r => r.categoryName.toLowerCase() === formData.categoryName.toLowerCase() && r.id !== selectedRecord?.id)) {
      newErrors.categoryName = 'Category Name must be unique';
    }
  ...
  const handleEdit = (record: MedicineCategoryRecord) => {
  ...
  const handleDeleteRequest = (record: MedicineCategoryRecord) => {
    setSelectedRecord(record);
      ...
      {!isFormOpen ? (
        <>
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-slate-800">Medicine Category Master</h1>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between">
              ...
              <button onClick={() => exportToExcel(records, 'MedicineCategoryMaster')} title="Export to Excel" className="p-2 border border-emerald-200 rounded-lg text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 transition-colors">
                  ...
                  {filteredRecords.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan={5} className="px-4 py-8 text-center text-slate-500">
                        {isLoading ? 'Loading records...' : 'No category records found matching your criteria.'}
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <>
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-slate-500 text-sm">Define a new medicine classification</p>
            </div>
          </div>
