...
export const DiagnosisMaster = () => {
  ...
  const filteredRecords = records.filter(record => {
    ...
    return matchesSearch && matchesIcd && matchesCategory;
  });
      ...
      {!isFormOpen ? (
        <>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-bold text-slate-800">Diagnosis</h1>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between">
              ...
              <button onClick={() => { setSearchTerm(''); setFilterCategory(''); setFilterIcdVersion(''); }} title="Clear search & filters" className="p-2 border border-red-200 rounded-lg text-red-500 hover:bg-red-50 hover:border-red-300 transition-colors">
                <X className="w-4 h-4" />
              </button>
              <button onClick={() => exportToExcel(records, 'DiagnosisMaster')} title="Export to Excel" className="p-2 border border-emerald-200 rounded-lg text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 transition-colors">
                  ...
                  {filteredRecords.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan={5} className="px-4 py-8 text-center text-slate-500">
                        {isLoading ? 'Loading records...' : 'No diagnosis records found matching your criteria.'}
                      </td>
                    </tr>
                  )}
                </tbody>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="text" value={formData.diagnosisCode} readOnly maxLength={10} className="w-full px-4 py-2 bg-slate-100 border border-slate-200 rounded-xl text-sm text-slate-500 cursor-not-allowed focus:outline-none" />
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.diagnosisName} onChange={e => setFormData({...formData, diagnosisName: e.target.value})} maxLength={50} className={`w-full px-4 py-2 bg-slate-50 border rounded-xl text-sm focus:outline-none focus:ring-2 transition-all ${errors.diagnosisName ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-primary/20'}`} />
