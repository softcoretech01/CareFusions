...
interface MinorOperationRecord {
  id: number;
...
const emptyData: Omit<MinorOperationRecord, 'id'> = {
  operationCode: '',
  operationName: '',
  department: '',
...
const mapApiToRecord = (item: any): MinorOperationRecord => ({
  id:                item.id,
...
export const MinorOperationMaster = () => {
  const [records, setRecords] = useState<MinorOperationRecord[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<MinorOperationRecord | null>(null);
  const [formData, setFormData] = useState<Omit<MinorOperationRecord, 'id'>>(emptyData);
  ...
  const fetchMinorOperations = async () => {
    try {
      setIsLoading(true);
      const res = await fetch(`${API_BASE}/minor-operations/`);
      if (!res.ok) throw new Error('Failed to fetch minor operation records');
  ...
  useEffect(() => {
    fetchMinorOperations();
    fetchDropdowns();
  }, []);

  const validateForm = () => {
    const newErrors: Record<string, string> = {};
    if (!formData.operationCode.trim()) newErrors.operationCode = 'Minor Operation Code is required';
    if (!formData.operationName.trim()) newErrors.operationName = 'Minor Operation Name is required';
    ...
    if (records.some(r => r.operationCode === formData.operationCode && r.id !== selectedRecord?.id)) {
      newErrors.operationCode = 'Minor Operation Code must be unique';
    }
  ...
  const handleEdit = (record: MinorOperationRecord) => {
  ...
  const handleView = (record: MinorOperationRecord) => {
    setSelectedRecord(record);
    setIsViewOpen(true);
  };

  const handleDeleteRequest = (record: MinorOperationRecord) => {
    setSelectedRecord(record);
  ...
  const handleSaveForm = async () => {
    ...
    try {
      ...
      const url = selectedRecord 
        ? `${API_BASE}/minor-operations/${selectedRecord.id}`
        : `${API_BASE}/minor-operations/`;
      ...
      if (!res.ok) {
        ...
        throw new Error(err.detail || 'Failed to save minor operation');
      }

      await fetchMinorOperations();
      setIsFormOpen(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/minor-operations/${selectedRecord.id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete minor operation');
      await fetchMinorOperations();
  ...
  return (
    ...
    >
      {!isFormOpen ? (
        <>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-bold text-slate-800">Minor Operation</h1>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  type="text"
                  placeholder="Search by Minor Operation Name or Code..."
                  value={searchTerm}
              ...
              <button onClick={() => exportToExcel(records, 'MinorOperationMaster')} title="Export to Excel" className="p-2 border border-emerald-200 rounded-lg text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 transition-colors">
            ...
            <div className="flex-1 overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-600">
                  <tr>
                    <th className="px-4 py-3 font-medium">Minor Operation Code</th>
                    <th className="px-4 py-3 font-medium">Minor Operation Name</th>
                  ...
                  {filteredRecords.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan={5} className="px-4 py-8 text-center text-slate-500">
                        {isLoading ? 'Loading records...' : 'No minor operation records found matching your criteria.'}
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="flex justify-between items-center mb-2">
            <div>
              <h1 className="text-2xl font-bold text-slate-800">
                {selectedRecord ? `Edit Minor Operation: ${selectedRecord.operationName}` : 'Add New Minor Operation'}
              </h1>
            </div>
          </div>
      ...
      <Modal
        ...
        title="View Minor Operation Details"
        maxWidth="lg"
      >
        {selectedRecord && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><span className="text-xs text-slate-400 block">Minor Operation Code</span><span className="text-sm font-medium">{selectedRecord.operationCode}</span></div>
              <div><span className="text-xs text-slate-400 block">Minor Operation Name</span><span className="text-sm font-medium">{selectedRecord.operationName}</span></div>
      ...
      >
        <div className="flex flex-col items-center text-center py-4">
          ...
          <p className="text-slate-500 text-sm mb-6">
            Are you sure you want to delete Minor Operation <span className="font-semibold text-slate-700">{selectedRecord?.operationName}</span>?
          </p>
