...
import {
  ...
  Save, RefreshCw, MessageSquare, X
} from 'lucide-react';
...
// TemplateCode is auto-generated by the backend (SMS-001 format)
type SmsTemplateForm = Omit<SmsTemplateRecord, 'id' | 'templateCode' | 'createdBy' | 'createdDate' | 'updatedBy' | 'updatedDate'>;

const emptyData: SmsTemplateForm = {
  templateName: '',
  module: '',
  event: '',
  description: '',
...
const LIMITS = {
  ...
  description: 500, smsSubject: 200, smsContent: 1000, remarks: 500,
};

const MODULES = ['Patient Registration', 'Appointment', 'Billing', 'Laboratory', 'Radiology'];
const EVENTS = ['Appointment Booked', 'Appointment Reminder', 'Payment Received', 'Lab Report Ready'];
const VARIABLES = ['{{PatientName}}', '{{DoctorName}}', '{{AppointmentDate}}', '{{AppointmentTime}}', '{{BillAmount}}', '{{HospitalName}}'];

const mapApiToRecord = (item: Record<string, unknown>): SmsTemplateRecord => ({
  ...
  smsSubject:   (item.smsSubject   as string) ?? '',
  smsContent:   item.smsContent   as string,
...
export const SmsMaster = () => {
  const [records, setRecords] = useState<SmsTemplateRecord[]>([]);
  ...
  const [formData, setFormData] = useState<SmsTemplateForm>(emptyData);
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      setApiError(err instanceof Error ? err.message : 'Failed to load SMS templates');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchNextCode = async () => {
    setNextCode('');
    try {
      const res = await fetch(`${API_BASE}/sms-templates/next-code`);
  ...
  const validateForm = () => {
    ...
    else if (formData.smsContent.length > LIMITS.smsContent) newErrors.smsContent = `Max ${LIMITS.smsContent} characters`;
  ...
  const insertVariable = (variable: string) => {
    setFormData(prev => {
      const next = prev.smsContent + variable;
      return { ...prev, smsContent: next.slice(0, LIMITS.smsContent) };
    });
  };
  ...
  const handleSaveForm = async () => {
    ...
    try {
      const body = {
        ...
        smsSubject:   formData.smsSubject || null,
      ...
      let res: Response;
      if (selectedRecord) {
        res = await fetch(`${API_BASE}/sms-templates/${selectedRecord.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...body, updatedBy: 'Admin' }),
        });
      } else {
        res = await fetch(`${API_BASE}/sms-templates/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const confirmDelete = async () => {
    ...
    setApiError(null);
    try {
      const res = await fetch(`${API_BASE}/sms-templates/${selectedRecord.id}`, { method: 'DELETE' });
      ...
      {!isFormOpen ? (
        <>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-bold text-slate-800">SMS Template</h1>
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between">
              ...
              <button onClick={() => exportToExcel(records, 'SmsMaster')} title="Export to Excel" className="p-2 border border-emerald-200 rounded-lg text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 transition-colors">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 space-y-6">
                    <div>
                      ...
                      <input type="text" maxLength={LIMITS.smsSubject} value={formData.smsSubject} onChange={e => setFormData({...formData, smsSubject: e.target.value})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />
                    </div>
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className={`text-xs ${formData.smsContent.length > LIMITS.smsContent ? 'text-red-500' : 'text-slate-500'}`}>
                          {formData.smsContent.length}/{LIMITS.smsContent} characters
                        </span>
                      </div>
                      <textarea
                        rows={6}
                        maxLength={LIMITS.smsContent}
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5">
                    ...
                    <p className="text-xs text-slate-500 mb-4">Click to insert a variable into your template content.</p>
