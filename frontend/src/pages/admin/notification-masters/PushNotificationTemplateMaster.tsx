...
// TemplateCode is auto-generated by the backend (PNT-001 format)
type PushTemplateForm = Omit<PushTemplateRecord, 'id' | 'templateCode' | 'createdBy' | 'createdDate' | 'updatedBy' | 'updatedDate'>;

const emptyData: PushTemplateForm = {
  templateName: '',
  module: '',
  event: '',
  notificationTitle: '',
...
const LIMITS = {
  ...
  notificationTitle: 150, notificationMessage: 500,
  clickAction: 100, deepLinkUrl: 255, remarks: 500,
};

const MODULES = ['Patient Registration', 'Appointment', 'Laboratory', 'Radiology'];
const EVENTS = ['Appointment Booked', 'Appointment Reminder', 'Lab Report Ready', 'Radiology Report Ready'];
const PRIORITIES = ['Low', 'Medium', 'High'];
const VARIABLES = ['{{PatientName}}', '{{DoctorName}}', '{{AppointmentDate}}'];

// Deep links: a scheme like app://... or https://... (validated only when present)
const DEEPLINK_RE = /^[a-zA-Z][a-zA-Z0-9+.\-]*:\/\/\S+$/;

const mapApiToRecord = (item: Record<string, unknown>): PushTemplateRecord => ({
  id:                  item.id                  as number,
  templateCode:        item.templateCode        as string,
  templateName:        item.templateName        as string,
  module:              item.module              as string,
  event:               item.event               as string,
  notificationTitle:   item.notificationTitle   as string,
  notificationMessage: item.notificationMessage as string,
  clickAction:         (item.clickAction         as string) ?? '',
  deepLinkUrl:         (item.deepLinkUrl         as string) ?? '',
...
export const PushNotificationTemplateMaster = () => {
  const [records, setRecords] = useState<PushTemplateRecord[]>([]);
  ...
  const [formData, setFormData] = useState<PushTemplateForm>(emptyData);
  ...
  const fetchTemplates = async () => {
    ...
    setApiError(null);
    try {
      const res = await fetch(`${API_BASE}/push-templates/`);
      if (!res.ok) throw new Error(`Server error: ${res.status}`);
      const data: Record<string, unknown>[] = await res.json();
      setRecords(data.map(mapApiToRecord));
    } catch (err: unknown) {
      setApiError(err instanceof Error ? err.message : 'Failed to load push templates');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchNextCode = async () => {
    setNextCode('');
    try {
      const res = await fetch(`${API_BASE}/push-templates/next-code`);
  ...
  const validateForm = () => {
    ...
    if (!formData.notificationMessage.trim()) newErrors.notificationMessage = 'Notification Message is required';
    else if (formData.notificationMessage.length > LIMITS.notificationMessage) newErrors.notificationMessage = `Max ${LIMITS.notificationMessage} characters`;

    if (formData.deepLinkUrl.trim() && !DEEPLINK_RE.test(formData.deepLinkUrl.trim()))
      newErrors.deepLinkUrl = 'Must include a scheme, e.g. app://patient';
  ...
  const insertVariable = (variable: string) => {
    setFormData(prev => {
      const next = prev.notificationMessage + variable;
      return { ...prev, notificationMessage: next.slice(0, LIMITS.notificationMessage) };
    });
  };
  ...
  const handleSaveForm = async () => {
    ...
    try {
      const body = {
        ...
        clickAction:         formData.clickAction || null,
        deepLinkUrl:         formData.deepLinkUrl || null,
      ...
      let res: Response;
      if (selectedRecord) {
        res = await fetch(`${API_BASE}/push-templates/${selectedRecord.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...body, updatedBy: 'Admin' }),
        });
      } else {
        res = await fetch(`${API_BASE}/push-templates/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const confirmDelete = async () => {
    ...
    setApiError(null);
    try {
      const res = await fetch(`${API_BASE}/push-templates/${selectedRecord.id}`, { method: 'DELETE' });
        ...
        <>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between">
              ...
              <button onClick={() => exportToExcel(records, 'PushNotificationTemplateMaster')} title="Export to Excel" className="p-2 border border-emerald-200 rounded-lg text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 transition-colors">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 space-y-6">
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className={`text-xs ${formData.notificationMessage.length > LIMITS.notificationMessage ? 'text-red-500' : 'text-slate-500'}`}>
                          {formData.notificationMessage.length}/{LIMITS.notificationMessage}
                        </span>
                      </div>
                      <textarea
                        rows={4}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div>
                        ...
                        <input type="text" maxLength={LIMITS.clickAction} value={formData.clickAction} onChange={e => setFormData({...formData, clickAction: e.target.value})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" placeholder="e.g. Open App" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Deep Link URL</label>
                        <input type="text" maxLength={LIMITS.deepLinkUrl} value={formData.deepLinkUrl} onChange={e => setFormData({...formData, deepLinkUrl: e.target.value})} className={`w-full px-4 py-2 bg-slate-50 border rounded-xl text-sm focus:outline-none focus:ring-2 transition-all ${errors.deepLinkUrl ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-primary/20'}`} placeholder="e.g. app://patient" />
                        {errors.deepLinkUrl && <p className="text-red-500 text-xs mt-1">{errors.deepLinkUrl}</p>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-5">
                    ...
                    <p className="text-xs text-slate-500 mb-4">Click to insert a variable into the Notification Message.</p>
