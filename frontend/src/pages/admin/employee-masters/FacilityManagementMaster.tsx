...
const mapApiToRecord = (item: any): FacilityManagementRecord => ({
  ...
  supervisor:       item.supervisor as string || '',
  remarks:          item.remarks as string || '',
  profilePhoto:         item.profilePhoto as string || '',
  idProof:              item.idProof as string || '',
  policeVerification:   item.policeVerification as string || '',
});
...
export const FacilityManagementMaster = () => {
  const [records, setRecords] = useState<FacilityManagementRecord[]>([]);
  ...
  const [formData, setFormData] = useState<Omit<FacilityManagementRecord, 'id'> & { profilePhoto?: string; idProof?: string; policeVerification?: string }>(emptyData);
  ...
  const fetchStaff = async () => {
    try {
      setIsLoading(true);
      const res = await fetch(`${API_BASE}/facility-management/`);
      if (!res.ok) throw new Error('Failed to fetch facility management staff');
  ...
  const validateForm = () => {
    ...
    if (!formData.employeeId.trim()) newErrors.employeeId = 'Employee ID is required';    if (!formData.name.trim()) newErrors.name = 'Name is required';
  ...
  const handleCreateNew = async () => {
    let nextCode = '';
    try {
      const res = await fetch(`${API_BASE}/facility-management/next-code`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleSaveForm = async () => {
    ...
    try {
      const payload = {
        ...
        policeVerification: formData.policeVerification || null,
        remarks:            formData.remarks || null,
        ...(selectedRecord ? { modifiedBy: 'Dr. John Doe' } : { createdBy: 'Dr. John Doe' }),
      };

      const url    = selectedRecord
        ? `${API_BASE}/facility-management/${selectedRecord.id}`
        : `${API_BASE}/facility-management/`;
      ...
      if (!res.ok) {
        ...
        throw new Error(err.detail || 'Failed to save staff');
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/facility-management/${selectedRecord.id}`, { method: 'DELETE' });
        ...
        <>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between">
              ...
              <button onClick={() => { setSearchTerm(''); setFilterCategory(''); setFilterShift(''); }} title="Clear search & filters" className="p-2 border border-red-200 rounded-lg text-red-500 hover:bg-red-50 hover:border-red-300 transition-colors">
                <X className="w-4 h-4" />
              </button>
              <button onClick={() => exportToExcel(records, 'FacilityManagementMaster')} title="Export to Excel" className="p-2 border border-emerald-200 rounded-lg text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 transition-colors">
        ...
        <>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.assignedArea} onChange={e => setFormData({...formData, assignedArea: e.target.value})} placeholder="e.g., ICU, Ward 3" maxLength={50} className={`w-full px-4 py-2 bg-slate-50 border rounded-xl text-sm focus:outline-none focus:ring-2 transition-all ${errors.assignedArea ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-primary/20'}`} />
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.supervisor} onChange={e => setFormData({...formData, supervisor: e.target.value})} maxLength={50} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    ...
                    { label: 'Police Verification (Optional)', field: 'policeVerification' },
                  ].map((doc, i) => (
            ...
            <div className="mt-6 flex items-center justify-between pt-6 border-t border-slate-100">
              <Button variant="outline" color="secondary" onClick={() => setFormData(selectedRecord ? selectedRecord : { ...emptyData, employeeCode: formData.employeeCode })} icon={RefreshCw}>
                Reset
              </Button>
              <div className="flex gap-3">
