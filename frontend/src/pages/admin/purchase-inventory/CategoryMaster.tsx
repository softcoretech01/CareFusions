...
export const CategoryMaster = () => {
  ...
  const [formData, setFormData] = useState<CategoryForm>(emptyData);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let res: Response;
      if (selectedRecord) {
        res = await fetch(`${API_BASE}/categories/${selectedRecord.id}`, {
          method: 'PUT',
    ...
    >
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => exportToExcel(records, 'CategoryMaster')} className="p-2 border border-emerald-200 rounded-lg text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 transition-colors" title="Export to Excel">
      ...
      <Modal isOpen={isDeleteOpen} onClose={() => setIsDeleteOpen(false)} title="Delete Record" size="sm">
        <div className="text-center py-4">
          ...
          <p className="text-slate-500 mb-6">Are you sure you want to delete <strong>{selectedRecord?.categoryName}</strong>? This action cannot be undone.</p>
