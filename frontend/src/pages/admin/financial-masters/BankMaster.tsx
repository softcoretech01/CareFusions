...
const emptyData: Omit<BankRecord, 'id'> = {
  bankCode: '',
  bankName: '',
  accountNumber: '',
  accountHolderName: '',
...
const mockData: BankRecord[] = [
  {
    id: 1,
    bankCode: 'BNK-001',
    bankName: 'HDFC Bank',
    accountNumber: '50100234567890',
    accountHolderName: 'CareFusions Hospital',
    branch: 'Main City Branch',
    ifscCode: 'HDFC0001234',
    swiftCode: 'HDFCINBXXXX',
    currency: 'INR',
  ...
  },
  {
    ...
    accountHolderName: 'CareFusions Trust',
    branch: 'South Wing',
...
export const BankMaster = () => {
  const [records, setRecords] = useState<BankRecord[]>(mockData);
  ...
  const [formData, setFormData] = useState<Omit<BankRecord, 'id'>>(emptyData);
  ...
  const validateForm = () => {
    ...
    // Uniqueness checks
    if (records.some(r => r.bankCode === formData.bankCode && r.id !== selectedRecord?.id)) {
      newErrors.bankCode = 'Bank Code must be unique';
    }
    if (records.some(r => r.accountNumber === formData.accountNumber && r.id !== selectedRecord?.id)) {
  ...
  const handleCreateNew = () => {
    ...
    setFormData({
      ...emptyData,
      bankCode: `BNK-${nextId.toString().padStart(3, '0')}`
    });
    setErrors({});
    setIsFormOpen(true);
  };
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Bank Code <span className="text-red-500">*</span></label>
                    <input type="text" value={formData.bankCode} readOnly className="w-full px-4 py-2 bg-slate-100 border border-slate-200 rounded-xl text-sm text-slate-500 cursor-not-allowed focus:outline-none" />
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.bankName} onChange={e => setFormData({...formData, bankName: e.target.value})} className={`w-full px-4 py-2 bg-slate-50 border rounded-xl text-sm focus:outline-none focus:ring-2 transition-all ${errors.bankName ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-primary/20'}`} />
                  ...
                  <div className="lg:col-span-2">
                    ...
                    <input type="text" value={formData.accountHolderName} onChange={e => setFormData({...formData, accountHolderName: e.target.value})} className={`w-full px-4 py-2 bg-slate-50 border rounded-xl text-sm focus:outline-none focus:ring-2 transition-all ${errors.accountHolderName ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-primary/20'}`} />
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.ifscCode} onChange={e => setFormData({...formData, ifscCode: e.target.value.toUpperCase()})} className={`w-full px-4 py-2 bg-slate-50 border rounded-xl text-sm focus:outline-none focus:ring-2 transition-all ${errors.ifscCode ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-primary/20'}`} />
              ...
              {/* Additional Details */}
              <section>
                <h3 className="text-lg font-bold text-slate-800 mb-4 border-b border-slate-100 pb-2">Additional Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">SWIFT Code</label>
                    <input type="text" value={formData.swiftCode} onChange={e => setFormData({...formData, swiftCode: e.target.value.toUpperCase()})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />
                  </div>
                  <div>
