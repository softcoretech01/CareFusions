...
export const Login = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (selectedRole === 'registration') {
    ...
    } else if (selectedRole === 'appointments') {
      navigate('/appointments');
    } else if (selectedRole === 'opd') {
      navigate('/opd');
    } else if (selectedRole === 'ipd') {
      navigate('/ipd');
    } else if (selectedRole === 'emr') {
      navigate('/emr');
      ...
      navigate('/lab');
    } else if (selectedRole === 'radiology') {
      navigate('/radiology');
  ...
  return (
    <div className="h-screen w-full flex bg-white overflow-hidden">
      ...
      <motion.div
        ...
        className="w-full lg:w-[45%] bg-white flex items-center justify-center p-8 lg:p-12 relative z-20 h-full overflow-y-auto overflow-x-hidden"
      >
        <motion.div 
        ...
        >
            <motion.div variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }} className="text-center mb-8">
              <motion.div 
                className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-primary/20"
              ...
              <p className="text-slate-500 mt-2 text-base">Sign in to access your CareFusions ERP account</p>
            </motion.div>

            <motion.div variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }} className="mb-8">
              <p className="text-sm font-bold text-slate-700 mb-4">Select Your Role</p>
              <motion.div 
            ...
            <motion.div variants={{ hidden: { opacity: 0 }, show: { opacity: 1 } }} className="relative flex items-center py-3">
            ...
            <motion.form variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }} onSubmit={handleLogin} className="space-y-5 mt-4">
              <div>
                ...
                <motion.div whileTap={{ scale: 0.995 }} className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all placeholder:text-slate-400 text-slate-700 text-base"
              ...
              <div>
                ...
                <motion.div whileTap={{ scale: 0.995 }} className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-12 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all placeholder:text-slate-400 text-slate-700 text-base"
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                type="submit"
                className="w-full bg-primary text-white font-bold py-3.5 rounded-xl mt-4 flex items-center justify-center gap-2 hover:bg-primary-hover transition-all shadow-lg shadow-primary/30 text-base"
              >
                Sign In
                <motion.span 
                  initial={{ x: 0 }}
            ...
            <motion.div variants={{ hidden: { opacity: 0 }, show: { opacity: 1 } }} className="mt-8 pt-6 border-t border-slate-100 text-center flex flex-col items-center">
              <p className="text-sm text-slate-500 font-medium">Need help? <a href="#" className="text-primary font-bold hover:underline">Contact Support</a></p>
            </motion.div>
          </motion.div>
      </motion.div>
    </div>
