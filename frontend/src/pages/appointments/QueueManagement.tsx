            ...
            <PatientCard key={item.id} item={item} actions={
              <>
                ...
                <button
                  ...
                  className="w-full py-1.5 px-2 rounded-xl text-xs font-semibold text-red-400 hover:bg-red-50 border border-red-100 transition-colors"
                >
                  Cancel
                </button>
              </>
        ...
        <Column title="Completed" count={completed.length} icon={CheckCircle} iconColor="text-green-500" headerBg="bg-green-50/50">
          ...
          {cancelled.length > 0 && (
            <div className="mt-3 border-t border-slate-200 pt-3">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 px-1">Cancelled</p>
              {cancelled.map(item => (
                <PatientCard key={item.id} item={item} actions={
