            ...
            <tbody className="divide-y divide-slate-100">
              {Object.values(stock.reduce((acc: Record<string, any>, r) => {
                ...
                <tr key={s.storeName} className="hover:bg-slate-50/70">
                  ...
                  <td className="px-4 py-3 text-right text-slate-600">{s.lots}</td>
                  <td className="px-4 py-3 text-right text-slate-700">{s.qty}</td>
                  <td className="px-4 py-3 text-right font-semibold text-slate-800">{inr(s.value)}</td>
                </tr>
              ))}
              {stock.length === 0 && (
